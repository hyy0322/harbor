...
const (
	...
	DC_GZTOBIAAS       = "gztobiaas"
	DC_HKCJ            = "hkcj"
	DC_HL              = "hl"
	DC_HLTOBIAAS       = "hltobiaas"
	DC_HY              = "hy"
	DC_IND             = "ind"
	DC_INDDP           = "inddp" // ind data persistence vdc used for storage components
	DC_JPSAAS          = "jpsaas"
	DC_KSRU            = "ksru"
	DC_LF              = "lf"
	DC_LFTOBIAAS       = "lftobiaas"
	DC_LQ              = "lq" // 灵丘机房
	DC_MALIVA          = "maliva"
	DC_QDTOB           = "qdtob"
	...
	for formerIdx, latterIdx := 0, 1; latterIdx < len(ipv4s); {
		...
		if former.end >= latter.start {
			...
			if former.end > latter.end {
				...
				tmp = append(tmp, former)
			}
			if formerIdx+1 != len(flatten) {
				tmp = append(tmp, flatten[formerIdx+1:]...)
			}
			flatten = append(flatten[:formerIdx], tmp...)
		} else {
			// 两网段相互独立
			formerIdx++
